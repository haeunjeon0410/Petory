        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Expanded(
                  child: _buildInfoItem("체중", "${profile['weight'] ?? '?'} kg"),
                ),
                Expanded(
                  child: _buildInfoItem("나이", "${profile['age'] ?? '?'}살"),
                ),
                Expanded(
                  child: _buildInfoItem(
                    "중성화",
                    (profile['isNeutered'] ?? false) ? "O" : "X",
                  ),
                ),
              ],
            ),
...
class WeightTrendCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    double diff = history.length > 1
        ? currentWeight - (history[history.length - 2]['weight'] as double)
        : 0;
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(22),
          ...
          LayoutBuilder(
            builder: (context, constraints) => GestureDetector(
              onTapDown: (details) {
                ...
                double chartWidth = constraints.maxWidth - paddingLeft - 10;
                double chartHeight = 160 - paddingBottom;
                double minW = history
                ...
                double xInterval = history.length > 1
                ...
                for (int i = 0; i < history.length; i++) {
                  ...
                  if ((details.localPosition.dx - x).abs() < 22) {
                    if (details.localPosition.dy > -20 &&
                        details.localPosition.dy < chartHeight + 40) {
                      NutritionDialogs.showEditDeleteDialog(
                        context: context,
                        petName: petName,
                        index: i,
                        onUpdate: onUpdate,
              ...
              child: SizedBox(
                ...
                child: history.isEmpty
                    ...
                    : CustomPaint(painter: MonotoneWeightChartPainter(history)),
              ),
            ),
          ),
...
class MonotoneWeightChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (history.isEmpty) return;
    const primaryColor = Color(0xFF44403B);
    const pointColor = Color(0xFF605A55);
