...
class _RecordPageState extends State<RecordPage> {
  ...
  void _deletePhoto(String petId, DateTime date, String path) {
    setState(() {
      final list = record.photos[petId]?[date];
      list?.remove(path);
      if (list != null && list.isEmpty) {
        record.photos[petId]?.remove(date);
      }
    });
  ...
  Future<void> _pickImage() async {
    ...
    if (image != null) {
      final key = _showAlbum
          ? record.normalizeDate(DateTime(_albumMonth.year, _albumMonth.month))
          : record.normalizeDate(_selectedDay);
